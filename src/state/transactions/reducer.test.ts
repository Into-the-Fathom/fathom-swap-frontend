...
describe('transaction reducer', () => {
  ...
  describe('addTransaction', () => {
    it('adds the transaction', () => {
      ...
      expect(txs[ChainId.XDC]).toBeTruthy()
      expect(txs[ChainId.XDC]?.['0x0']).toBeTruthy()
      const tx = txs[ChainId.XDC]?.['0x0']
  ...
  describe('finalizeTransaction', () => {
    ...
    it('sets receipt', () => {
      ...
      const tx = store.getState()[ChainId.AXDC]?.['0x0']
      expect(tx?.summary).toEqual('hello world')
  ...
  describe('clearAllTransactions', () => {
    it('removes all transactions for the chain', () => {
      ...
      expect(Object.keys(store.getState())).toEqual([String(ChainId.XDC), String(ChainId.AXDC)])
      expect(Object.keys(store.getState()[ChainId.XDC] ?? {})).toEqual(['0x0'])
      expect(Object.keys(store.getState()[ChainId.AXDC] ?? {})).toEqual(['0x1'])
      store.dispatch(clearAllTransactions({ chainId: ChainId.XDC }))
      expect(Object.keys(store.getState())).toHaveLength(2)
      expect(Object.keys(store.getState())).toEqual([String(ChainId.XDC), String(ChainId.AXDC)])
      expect(Object.keys(store.getState()[ChainId.XDC] ?? {})).toEqual([])
      expect(Object.keys(store.getState()[ChainId.AXDC] ?? {})).toEqual(['0x1'])
    })
  })
})
