...
import { fathomBunniesAddress } from 'views/Nft/market/constants'
...
/**
 * Fetch market data for FathomBunnies NFTs by bunny id using the Subgraph
...
export const getNftsByBunnyIdSg = async (
  ...
  try {
    ...
    const res = await request(
      ...
      `,
      {
        collectionAddress: fathomBunniesAddress.toLowerCase(),
        where,
        orderDirection,
      },
    )
...
export const getAllFathomBunniesLowestPrice = async (bunnyIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleFathomBunnySubQueries = bunnyIds.map(
      (
        bunnyId,
    ...
    const rawResponse: Record<string, { currentAskPrice: string }[]> = await request(
      GRAPH_API_NFTMARKET,
      gql`
        query getAllFathomBunniesLowestPrice {
          ${singleFathomBunnySubQueries}
        }
      `,
    )
    return fromPairs(
    ...
    console.error('Failed to fetch FathomBunnies lowest prices', error)
    return {}
  }
}

export const getAllFathomBunniesRecentUpdatedAt = async (bunnyIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleFathomBunnySubQueries = bunnyIds.map(
      (
        bunnyId,
    ...
    console.error('Failed to fetch FathomBunnies latest market updates', error)
    return {}
  }
}
...
export const getMetadataWithFallback = (apiMetadata: ApiResponseCollectionTokens['data'], bunnyId: string) => {
  ...
  return (
    apiMetadata[bunnyId] ?? {
      name: '',
      description: '',
      collection: { name: 'Fathom Bunnies' },
      image: {
        original: '',
        thumbnail: '',
      },
...
export const getFathomBunniesAttributesField = (bunnyId: string) => {
