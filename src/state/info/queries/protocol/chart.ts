...
import { FathomDayDatasResponse } from '../types'

/**
 * Data for displaying Liquidity and Volume charts on Overview page
 */
const FATHOM_DAY_DATAS = gql`
  query overviewCharts($startTime: Int!, $skip: Int!) {
    fathomDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      date
      dailyVolumeUSD
      totalLiquidityUSD
    }
...
const getOverviewChartData = async (skip: number): Promise<{ data?: ChartEntry[]; error: boolean }> => {
  try {
    const { fathomDayDatas } = await infoClient.request<FathomDayDatasResponse>(FATHOM_DAY_DATAS, {
      startTime: PCS_V2_START,
      skip,
    })
    const data = fathomDayDatas.map(mapDayData)
