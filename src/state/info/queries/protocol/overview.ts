...
const formatFathomFactoryResponse = (rawFathomFactory?: FathomFactory) => {
  if (rawFathomFactory) {
    return {
      totalTransactions: parseFloat(rawFathomFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawFathomFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawFathomFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatFathomFactoryResponse(data?.fathomFactories?.[0])
      const overviewData24 = formatFathomFactoryResponse(data24?.fathomFactories?.[0])
      const overviewData48 = formatFathomFactoryResponse(data48?.fathomFactories?.[0])
