...
import { FathomProfile } from 'config/abi/types/FathomProfile'
...
const transformProfileResponse = (
  profileResponse: Awaited<ReturnType<FathomProfile['getUserProfile']>>,
): Partial<Profile> => {
...
export const getProfile = async (address: string): Promise<GetProfileResponse> => {
  try {
    const profileCalls = ['hasRegistered', 'getUserProfile'].map((method) => {
      return { address: getFathomProfileAddress(), name: method, params: [address] }
    })
