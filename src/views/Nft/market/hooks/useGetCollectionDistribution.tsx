...
export const useGetCollectionDistributionPB = () => {
  ...
  useEffect(() => {
    const fetchTokens = async () => {
      ...
      try {
        apiResponse = await getNftsFromCollectionApi(fathomBunniesAddress)
        if (!apiResponse) {
      ...
      const bunnyCountCalls = tokenIds.map((tokenId) => ({
        address: getFathomBunniesAddress(),
        name: 'bunnyCount',
        params: [tokenId],
      }))
      try {
        const response = await multicallv2({ abi: fathomBunniesAbi, calls: bunnyCountCalls })
