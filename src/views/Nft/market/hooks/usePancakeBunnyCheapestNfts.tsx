...
import {
  ...
  getFathomBunniesAttributesField,
...
const fetchCheapestBunny = async (
  ...
  const lowestPriceUpdatedBunny = await getLowestUpdatedToken(fathomBunniesAddress.toLowerCase(), nftsMarketTokenIds)
  ...
  const cheapestBunnyOfAccount = nftsMarket
    ...
    .map((marketData) => {
      ...
      const attributes = getFathomBunniesAttributesField(marketData.otherId)
...
export const useFathomBunnyCheapestNft = (bunnyId: string, nftMetadata: ApiResponseCollectionTokens) => {
  const { account } = useWeb3React()
  const { data, status, mutate } = useSWR(
    nftMetadata && bunnyId ? ['cheapestBunny', bunnyId, account] : null,
    async () => {
      const allCheapestBunnyClause = {
        collection: fathomBunniesAddress.toLowerCase(),
        otherId: bunnyId,
        isTradable: true,
      }
      if (!account) {
