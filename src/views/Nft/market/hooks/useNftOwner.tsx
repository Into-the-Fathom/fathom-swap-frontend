...
const useNftOwner = (nft: NftToken, isOwnNft = false) => {
  ...
  const fathomProfileAddress = getFathomProfileAddress()
  ...
  useEffect(() => {
    const getOwner = async () => {
      try {
        if (isOwnNft && account) {
          setOwner(account)
        } else if (tokenOwner && tokenOwner.toLowerCase() !== fathomProfileAddress.toLowerCase()) {
          setOwner(tokenOwner)
        } else {
          setOwner(null)
        }
  ...
  }, [account, isOwnNft, currentSeller, collectionContract, tokenId, tokenOwner, fathomProfileAddress])
