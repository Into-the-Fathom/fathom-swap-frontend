...
import { ArrowBackIcon, ArrowForwardIcon, Box, IconButton, Text, Flex, useMatchBreakpoints } from '@fathomswap/uikit'
...
const MoreFromThisCollection: React.FC<React.PropsWithChildren<MoreFromThisCollectionProps>> = ({
  ...
  const allFathomBunnyNfts = useAllFathomBunnyNfts(collectionAddress)

  const isPBCollection = isAddress(collectionAddress) === fathomBunniesAddress
  ...
  let nftsToShow = useMemo(() => {
    return shuffle(
      allFathomBunnyNfts
        ? allFathomBunnyNfts.filter((nft) => nft.name !== currentTokenName)
        : collectionNfts?.filter((nft) => nft.name !== currentTokenName && nft.marketData?.isTradable),
    )
  }, [allFathomBunnyNfts, collectionNfts, currentTokenName])
  ...
  if (isPBCollection) {
    // FathomBunnies should display 1 card per bunny id
