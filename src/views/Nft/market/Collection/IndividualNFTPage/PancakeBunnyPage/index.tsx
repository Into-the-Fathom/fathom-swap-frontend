...
import { Flex } from '@fathomswap/uikit'
...
import MainFathomBunnyCard from './MainFathomBunnyCard'
...
import { useFathomBunnyCheapestNft } from '../../../hooks/useFathomBunnyCheapestNfts'
import ManageNftsCard from '../shared/ManageNFTsCard'

interface IndividualFathomBunnyPageProps {
  bunnyId: string
}

const IndividualFathomBunnyPage = (props: IndividualFathomBunnyPageProps) => {
  const collection = useGetCollection(fathomBunniesAddress)

  if (!collection) {
    return <PageLoader />
  }

  return <IndividualFathomBunnyPageBase {...props} />
}

const IndividualFathomBunnyPageBase: React.FC<React.PropsWithChildren<IndividualFathomBunnyPageProps>> = ({
  bunnyId,
}) => {
  const collection = useGetCollection(fathomBunniesAddress)
  ...
  } = useFathomBunnyCheapestNft(bunnyId, nftMetadata)

  const { data: distributionData, isFetching: isFetchingDistribution } = useGetCollectionDistributionPB()

  useEffect(() => {
    const fetchNftMetadata = async () => {
      const metadata = await getNftsFromCollectionApi(fathomBunniesAddress)
    ...
    <Page>
      ...
      <TwoColumnsContainer flexDirection={['column', 'column', 'column', 'row']}>
        <Flex flexDirection="column" width="100%">
          ...
          <DetailsCard
            contractAddress={fathomBunniesAddress}
...
export default IndividualFathomBunnyPage
