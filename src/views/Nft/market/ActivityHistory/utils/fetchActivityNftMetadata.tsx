...
export const fetchActivityNftMetadata = async (activities: Activity[]): Promise<NftToken[]> => {
  const hasPBCollections = activities.some(
    (activity) => activity.nft.collection.id.toLowerCase() === fathomBunniesAddress.toLowerCase(),
  )

  const activityNftTokenIds = uniqBy(
    activities
      .filter((activity) => activity.nft.collection.id.toLowerCase() !== fathomBunniesAddress.toLowerCase())
  ...
  const [bunniesMetadata, nfts] = await Promise.all([
    hasPBCollections ? getNftsFromCollectionApi(fathomBunniesAddress) : Promise.resolve(null),
