...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text, useToast } from '@fathomswap/uikit'
...
const PauseProfilePage: React.FC<React.PropsWithChildren<PauseProfilePageProps>> = ({ onDismiss, onSuccess }) => {
  ...
  const fathomProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = async () => {
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(fathomProfileContract, 'pauseProfile')
    })
    if (receipt?.status) {
      // Re-fetch profile
      refreshProfile()
