...
import { Text, Flex, AccountIcon, TeamBattleIcon, Box, useTooltip, LinkExternal } from '@fathomswap/uikit'
...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%Fathom Squad NFT%'

  const rawText = t(`Set %Fathom Squad NFT% as Fathom Profile avatar`)
      ...
      element={
        <>
          <LinkExternal
            ...
            href={`https://fathomswap.finance/nfts/collections/${admissionProfile}`}
          >
            {t('Fathom Squad NFT')}
          </LinkExternal>
          <br />
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%Fathom Profile%'

  const rawText = t(`Reach %point% or more %Fathom Profile% points`, { point: pointThreshold })
    ...
    <TransWithElement
      ...
      element={
        <>
          ...
          <LinkExternal
            style={{ display: 'inline' }}
            href={`https://fathomswap.finance/profile/${account}/achievements/`}
          >
            {t('Fathom Profile')}
          </LinkExternal>
        </>
