...
import { ADMINS, FATHOM_SPACE, SNAPSHOT_VERSION } from './config'
...
export const getVotingPower = async (
  ...
  const [total] = await getScores(FATHOM_SPACE, STRATEGIES, NETWORK, [account], blockNumber)
...
export async function getVotingPowerByCakeStrategy(voters: string[], blockNumber: number) {
  const strategyResponse = await getScores(FATHOM_SPACE, STRATEGIES, NETWORK, voters, blockNumber)
