import { FACTORY_ADDRESS } from '@fathomswap/sdk'
...
export const getStaticProps: GetStaticProps = async () => {
  const totalTxQuery = gql`
    query TotalTransactions($id: ID!, $block: Block_height) {
      fathomFactory(id: $id, block: $block) {
        totalTransactions
      }
    }
  `
  ...
  if (process.env.SF_HEADER) {
    try {
      ...
      if (
        totalTx?.fathomFactory?.totalTransactions &&
        totalTx30DaysAgo?.fathomFactory?.totalTransactions &&
        parseInt(totalTx.fathomFactory.totalTransactions) > parseInt(totalTx30DaysAgo.fathomFactory.totalTransactions)
      ) {
        results.totalTx30Days =
          parseInt(totalTx.fathomFactory.totalTransactions) -
          parseInt(totalTx30DaysAgo.fathomFactory.totalTransactions)
      }
    } catch (error) {
  ...
  const usersQuery = gql`
    query userCount($since: ISO8601DateTime, $till: ISO8601DateTime) {
      ethereum(network: bsc) {
        dexTrades(exchangeName: { in: ["Fathom", "Fathom v2"] }, date: { since: $since, till: $till }) {
          count(uniq: senders)
        }
      }
    }
  ...
  try {
    ...
    `)
    const { totalLiquidityUSD } = result.fathomFactories[0]
