...
import { Text, Flex, Box, CloseIcon, IconButton, useMatchBreakpoints } from '@fathomswap/uikit'
...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = t("please make sure you're visiting https://fathomswap.finance - check the URL carefully.")
    return warningText.split(/(https:\/\/fathomswap.finance)/g)
  }, [t])
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://fathomswap.finance'}
          color={text === 'https://fathomswap.finance' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
